...
{
  return swell_app_is_inactive ? 1 : (gdk_options & OPTION_ALLOW_MAYBE_INACTIVE) && s_deactivate_timer != 0 ? -1
                                                                                                            : 0;
}
...
{
  if (!hwnd || !hwnd->m_oswindow || !(hwnd->m_style & WS_CAPTION))
    return;
...
void SWELL_initargs(int *argc, char ***argv)
{
  if (!SWELL_gdk_active)
  {
    ...
    if (SWELL_gdk_active > 0)
    {
      ...
      if (pb)
        s_program_icon_list = g_list_append(s_program_icon_list, pb);
...
static int swell_gdk_option(const char *name, const char *defstr, int defv)
{
  ...
  if (!buf[0])
    WritePrivateProfileString(".swell", name, defstr, "");
  if (buf[0] >= '0' && buf[0] <= '9')
    return atoi(buf);
  return defv;
}
...
static void init_options()
{
  if (!gdk_options)
  {
    ...
    {
    case 1:
      gdk_options |= OPTION_BORDERLESS_DIALOG;
      break;
    case 2:
    ...
    {
    case -1:
      if (!wmname || strnicmp(wmname, "KWin", 4))
        break;
      // fall through
    case 1:
    ...
    else
    {
      if (swell_initwindowsys())
      {
        ...
        if (hwnd->m_owner && (gdk_options & OPTION_KEEP_OWNED_ABOVE))
        {
          ...
          while (own->m_parent && !own->m_oswindow)
            own = own->m_parent;

          if (!own->m_oswindow)
          {
            if (!IsModalDialogBox(hwnd))
              return; // defer
...
void swell_oswindow_updatetoscreen(HWND hwnd, RECT *rect)
{
  ...
  if (hwnd && hwnd->m_backingstore && hwnd->m_oswindow)
  {
    ...
    if (temp_surface)
      bm->Extended(0xca140, temp_surface); // release
...
#undef DEF_GK2
  ...
  case DEF_GKY(KP_9):
    return VK_NUMPAD9;
  case DEF_GKY(KP_Multiply):
    return VK_MULTIPLY;
  case DEF_GKY(KP_Add):
    return VK_ADD;
  case DEF_GKY(KP_Separator):
    return VK_SEPARATOR;
  case DEF_GKY(KP_Subtract):
    return VK_SUBTRACT;
  case DEF_GKY(KP_Decimal):
    return VK_DECIMAL;
  case DEF_GKY(KP_Divide):
    return VK_DIVIDE;
  case DEF_GKY(Num_Lock):
    return VK_NUMLOCK;
  case DEF_GKY(KP_Begin):
    return VK_SELECT;
  }
  return 0;
}
...
static GdkAtom utf8atom()
{
  static GdkAtom tmp;
  if (!tmp)
    tmp = gdk_atom_intern_static_string("UTF8_STRING");
  return tmp;
}
static GdkAtom tgtatom()
{
  static GdkAtom tmp;
  if (!tmp)
    tmp = gdk_atom_intern_static_string("TARGETS");
  return tmp;
}
static GdkAtom urilistatom()
{
  static GdkAtom tmp;
  if (!tmp)
    tmp = gdk_atom_intern_static_string("text/uri-list");
  return tmp;
}
  ...
  if (swell_dragsrc_osw && b->window == swell_dragsrc_osw)
  {
    if (swell_dragsrc_hwnd)
    {
      if (b->target == tgtatom())
      {
        ...
        if (!pw)
          pw = gdk_window_foreign_new(b->requestor);
#else
...
static guint find_compose_sequence_compact(const guint16 *state, int state_size) // returns 0 if sequence not found, 1 if partial match, otherwise unicode character
{
  ...
  if (WDL_NOT_NORMALLY(state_size > max_seq_len))
    return 0;
...
{
  if (state_size < 2)
    return state_size == 1 && IS_DEAD_KEY(state[0]) ? 1 : 0;
  int i;
  for (i = 0; i < state_size && IS_DEAD_KEY(state[i]); i++)
    ;
  if (i != state_size - 1)
    return 0; // must be all dead keys + one non-dead
...
#define CASE(keysym, unicode)                \
  case DEF_GKY(dead_##keysym):               \
    g_string_append_unichar(input, unicode); \
    break
      CASE(grave, 0x0300);
      CASE(acute, 0x0301);
...
static guint swell_gdkComposeKeys(GdkEventKey *k) // return 0 if not in composition, 1 if mid-composition, >1 if composed
{
  ...
  if (!IS_DEAD_KEY(k->keyval) && !state_pos)
    return 0;
  ...
  if (rv == 0)
    rv = find_compose_sequence_alg(state, state_pos);
  if (rv != 1 || WDL_NOT_NORMALLY(state_pos >= state_max))
    state_pos = 0;
  return rv ? rv : 1;
}
...
static void OnKeyEvent(GdkEventKey *k)
{
  ...
  if (k->state & GDK_MOD1_MASK)
    modifiers |= FALT;
  if (k->state & SWELL_WINDOWSKEY_GDK_MASK)
    modifiers |= FLWIN;
  ...
  else
  {
    ...
    else
    {
      ...
      {
        if (kv == DEF_GKY(Shift_L) || kv == DEF_GKY(Shift_R))
          kv = VK_SHIFT;
        else if (kv == DEF_GKY(Control_L) || kv == DEF_GKY(Control_R))
          kv = VK_CONTROL;
        else if (kv == DEF_GKY(Alt_L) || kv == DEF_GKY(Alt_R))
          kv = VK_MENU;
        else if (kv == DEF_GKY(Super_L) || kv == DEF_GKY(Super_R))
          kv = VK_LWIN;
        else
          return; // unknown modifie key
  ...
  if (foc && IsChild(hwnd, foc))
    hwnd = foc;
  else if (foc && foc->m_oswindow && !(foc->m_style & WS_CAPTION))
    hwnd = foc; // for menus, event sent to other window due to gdk_window_set_override_redirect()
...
static HWND getMouseTarget(SWELL_OSWINDOW osw, POINT p, const HWND *hwnd_has_osw)
{
  ...
  if (!hwnd || swell_window_wants_all_input() == hwnd)
    return hwnd;
...
static void OnScrollEvent(GdkEventScroll *b)
{
  ...
  if (hwnd)
  {
    ...
    int msg = (b->direction == GDK_SCROLL_UP || b->direction == GDK_SCROLL_DOWN) ? WM_MOUSEWHEEL : (b->direction == GDK_SCROLL_LEFT || b->direction == GDK_SCROLL_RIGHT) ? WM_MOUSEHWHEEL
                                                                                                                                                                         : 0;
...
bool swell_gdk_set_fullscreen(HWND hwnd, int fs) // fs=2 if window might have owned children
{
  if (!hwnd)
    return false;
  if (fs == 2 && (gdk_options & OPTION_FULLSCREEN_FOR_OWNER_WINDOWS) != OPTION_FULLSCREEN_FOR_OWNER_WINDOWS)
    return false;
...
int SWELL_SetWindowLevel(HWND hwnd, int newlevel)
{
  ...
  if (hwnd)
  {
    ...
    if (hwnd->m_oswindow)
      gdk_window_set_keep_above(hwnd->m_oswindow, newlevel > 0 && !swell_app_is_inactive);
  }
  return rv;
}
...
void SWELL_GetViewPort(RECT *r, const RECT *sourcerect, bool wantWork)
{
  ...
  for (gint idx = 0; idx < n; idx++)
  {
    ...
    {
      int dx = 0, dy = 0;
      if (tmp.left > sr.right)
        dx = tmp.left - sr.right;
      else if (tmp.right < sr.left)
        dx = sr.left - tmp.right;
      if (tmp.bottom < sr.top)
        dy = tmp.bottom - sr.top;
      else if (tmp.top > sr.bottom)
        dy = tmp.top - sr.bottom;
      score = -(dx * dx + dy * dy);
    }
...
void swell_oswindow_resize(SWELL_OSWINDOW wnd, int reposflag, RECT f)
{
  ...
  if ((reposflag & 3) == 3)
  {
    ...
    gdk_window_resize(wnd, f.right - f.left, f.bottom - f.top);
  else if (reposflag & 1)
    gdk_window_move(wnd, f.left, f.top);
}
...
void swell_oswindow_postresize(HWND hwnd, RECT f)
{
  if (hwnd->m_oswindow && (hwnd->m_oswindow_private & PRIVATE_NEEDSHOW) && !hwnd->m_oswindow_fullscreen)
  {
    gdk_window_show(hwnd->m_oswindow);
    if (hwnd->m_style & WS_CAPTION)
      gdk_window_unmaximize(hwnd->m_oswindow);     // fixes Kwin
...
{
  if (s_clipboard_getstate_fmt == type)
    return s_clipboard_getstate;

  HWND h = s_clip_hwnd;
  while (h && !h->m_oswindow)
    h = h->m_parent;
...
UINT EnumClipboardFormats(UINT lastfmt)
{
  ...
  {
    int fmt = 0;
    if (!m_clip_recs.Enumerate(x++, &fmt))
      return 0;
    if (lastfmt == 0)
      return fmt;

    if ((UINT)fmt == lastfmt)
      return m_clip_recs.Enumerate(x++, &fmt) ? fmt : 0;
  }
}
...
void SetClipboardData(UINT type, HANDLE h)
{
  ...
  if (h)
    m_clip_recs.Insert(type, h);
  else
    m_clip_recs.Delete(type);
}
...
UINT RegisterClipboardFormat(const char *desc)
{
  ...
  for (x = 0; x < n; x++)
    if (!strcmp(m_clip_curfmts.Get(x), desc))
      return x + 1;
...
WORD GetAsyncKeyState(int key)
{
  if (SWELL_gdk_active > 0)
  {
    ...
    if (key == VK_LBUTTON)
      return ((mod & GDK_BUTTON1_MASK) || g_swell_touchptr) ? 0x8000 : 0;
    if (key == VK_MBUTTON)
      return (mod & GDK_BUTTON2_MASK) ? 0x8000 : 0;
    if (key == VK_RBUTTON)
      return (mod & GDK_BUTTON3_MASK) ? 0x8000 : 0;

    if (key == VK_CONTROL)
      return (mod & GDK_CONTROL_MASK) ? 0x8000 : 0;
    if (key == VK_MENU)
      return (mod & GDK_MOD1_MASK) ? 0x8000 : 0;
    if (key == VK_SHIFT)
      return (mod & GDK_SHIFT_MASK) ? 0x8000 : 0;
    if (key == VK_LWIN)
      return (mod & SWELL_WINDOWSKEY_GDK_MASK) ? 0x8000 : 0;
  }
  return 0;
}
      ...
      if (r.right > 0 && r.bottom > 0 && bs)
      {
        ...
        if (XQueryTree(bs->native_disp, bs->native_w, &root, &par, &list, &nlist))
        {
          ...
          if (hints)
          {
            ...
            {
              if (r.right < hints->min_width)
                r.right = hints->min_width;
              if (r.bottom < hints->min_height)
                r.bottom = hints->min_height;
            }
...
static void forward_x11_drag_message(int gdkmsg, GdkEventDND *gdkevent, Window new_target)
{
  ...
  case GDK_DRAG_ENTER:
    ...
    xev.data.l[1] = (3 << 24); // protocol version 3
    {
      ...
      if (cnt > 3)
        xev.data.l[1] |= 1; // more types available that we couldn't fit
    }
    break;
  case GDK_DRAG_LEAVE:
...
{
  if (WDL_NOT_NORMALLY(!hwnd || !hwnd->m_oswindow || !xw))
    return false;
  ...
  for (int x = 0; x < filter_windows.GetSize(); x++)
  {
    ...
    if (bs->cur_parent != hwnd->m_oswindow)
      continue;
...
{
  if (WDL_NOT_NORMALLY(!hwnd || !hwnd->m_oswindow))
    return false;
  POINT pt = {xpos, ypos};
  Display *dpy = gdk_x11_display_get_xdisplay(gdk_window_get_display(hwnd->m_oswindow));
  Window new_target = 0;
  for (int x = 0; !new_target && x < filter_windows.GetSize(); x++)
  {
    ...
    if (bs->cur_parent != hwnd->m_oswindow)
      continue;
    if (!IsWindowVisible(bs->hwnd_child))
      continue;
    ...
    if (XQueryTree(dpy, bs->native_w, &root, &par, &list, &nlist) && list)
    {
      for (unsigned int i = 0; !new_target && i < nlist; i++)
      {
        int lx = 0, ly = 0;
        Window cr = 0;
        if (!XTranslateCoordinates(dpy, root, list[i], xpos, ypos, &lx, &ly, &cr))
          continue;
...
static bool OnDragEventDelegate(GdkEvent *evt)
{
  ...
  case GDK_DRAG_LEAVE:
    if (s_last_hwnd && s_last_hwnd == hwnd)
    {
      ...
      else if (SWELL_DDrop_onDragLeave)
          ...
          (xievent->evtype == XI_KeyPress || xievent->evtype == XI_KeyRelease))
      {
        ...
        if (want_key_embed_redirect(disp, xievent->event - 1, &dest, xievent->detail, xievent->mods.effective))
        {
          ...
          if (xievent->evtype == XI_KeyPress)
            k.xkey.type = KeyPress;
          else
            k.xkey.type = KeyRelease;
...
struct dropSourceInfo
{
  ...
  ~dropSourceInfo()
  {
    ...
    if (dragctx)
    {
      if (_gdk_drag_drop_done)
        _gdk_drag_drop_done(dragctx, state != 0);
...
static LRESULT WINAPI dropSourceWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_LBUTTONUP:
    if (inf->dragctx && !inf->state)
    {
      ...
      if (sel)
        gdk_selection_owner_set(swell_dragsrc_osw, sel, GDK_CURRENT_TIME, TRUE);
      ...
      if (evt->target == urilistatom())
      {
        ...
        if (s.GetLength())
        {
          *aOut = evt->property;
          ...
          if (!pw)
            pw = gdk_window_foreign_new(evt->requestor);
#else
...
void SWELL_InitiateDragDrop(HWND hwnd, RECT *srcrect, const char *srcfn, void (*callback)(const char *dropfn))
{
  ...
  while (GetCapture() == h)
  {
    ...
    Sleep(10);
    if (swell_dragsrc_timeout_start && (GetTickCount() - swell_dragsrc_timeout_start) > 500)
      ReleaseCapture();
  }
...
{
  if (s_last_setcursor == curs && SWELL_focused_oswindow == s_last_setcursor_oswnd)
    return;
...
{
  if (g_swell_mouse_relmode || g_swell_touchptr)
    return false;
...
HCURSOR SWELL_LoadCursor(const char *_idx)
{
  ...
  if (_idx == IDC_NO)
    def = GDK_PIRATE;
  else if (_idx == IDC_SIZENWSE)
    def = GDK_BOTTOM_LEFT_CORNER;
  else if (_idx == IDC_SIZENESW)
    def = GDK_BOTTOM_RIGHT_CORNER;
  else if (_idx == IDC_SIZEALL)
    def = GDK_FLEUR;
  else if (_idx == IDC_SIZEWE)
    def = GDK_RIGHT_SIDE;
  else if (_idx == IDC_SIZENS)
    def = GDK_TOP_SIDE;
  else if (_idx == IDC_ARROW)
    def = GDK_LEFT_PTR;
  else if (_idx == IDC_HAND)
    def = GDK_HAND1;
  else if (_idx == IDC_UPARROW)
    def = GDK_CENTER_PTR;
  else if (_idx == IDC_IBEAM)
    def = GDK_XTERM;
  else
  {
...
BOOL EnumDisplayMonitors(HDC hdc, const LPRECT r, MONITORENUMPROC proc, LPARAM lParam)
{
  ...
  for (int x = 0; x < nmon; x++)
  {
    ...
    if (!proc((HMONITOR)(INT_PTR)(x + 1), hdc, &screen_rect, lParam))
      break;
  }
  return TRUE;
}
...
{
  if (hwnd && !strcmp(type, "GdkWindow"))
    return hwnd->m_oswindow;
  return NULL;
}
